...
;;;;
;;;  -----RATES:
(include @FOLDER/erb6_rcpt_rates)
...
;; now we can define reactions like receptor internalization:
;;
;; {[[erbb1 1 * * *][egf 1]] @ :cell-membrane @ cell ->>
;;  [[erbb1 1 * * *][egf 1]] @ :inverse-endo-membrane]}
...
cell.dish.(contains [egf])
;cell.dish.(contains [egf] [hrg])
cell.cytoplasm.(contains [ATP])
;cell.cytoplasm.(contains [ATP] [ADP] [DEP1] [DEP2] [DEP3] [DEP4])
;cell.cytoplasm.(contains [ATP]   [ADP]  [DEP1] [DEP2] [DEP3] [DEP4] [SHC]  [SHCPASE] [GRB2] [GBSP] [SOS] [GAP] [RAF] 
;			 [iRAFK] [MEK]  [ERK]  [PP2A] [MKP]  [PI3K] [PTEN] [AKT]     [PDK])
cell.cell-membrane.(contains [erbB1])
;cell.cell-membrane.(contains [erbB1] [erbB2] [erbB3] [erbB4])
;cell.cell-membrane.(contains [erbB1] [erbB2] [erbB3] [erbB4] [RAS] [PtdIns])
; cell.endo-membrane.(contains [erbB1] [erbB2] [erbB3] [erbB4])
...
(format t "CELL DEFINED~%")
;;; ----------------------------------------------------------------------------
;;; RECEPTOR-LEVEL EVENTS 
;;; ----------------------------------------------------------------------------

;; LIGAND BINDS TO ERBB MONOMERS:  R + L <-> R-L
...
(with-data-table 
 ...
 ((        egf                ) ; hrg               )
  ...
  ;(erbb2          _          ))
  ;(erbb3    _                 (KLbindF KLbindR)  ) 
  ;(erbb4    _                 (KLbindF KLbindR)  )) 
 ;; FORWARD (BINDING)
...
(with-data-table 
    ...
    ((             (erbb1 *)     ); (erbb2 nil)) (erbb3 *)     (erbb4 *)     )
     ((erbb1   *)  (KdimF KdimR) )) ; (KdimF KdimR))  (KdimF KdimR) (KdimF KdimR) )
					;((erbb2 NIL)  (KdimF KdimR)       _      )) ;(KdimF KdimR) (KdimF KdimR) )
					;((erbb3   *)        _             _             _       (KdimF KdimR) )
					;((erbb4   *)        _             _             _       (KdimF KdimR) ))
  ;;
...
;(with-data-table (:rows $R :cols $P :cells ($K1f $K1r $K2f) :ignore _)
;  ((        dep1                     dep2                     dep3                     dep4         )
;   (erbb1  (KpbindF KpbindR KDepho1C)_                        _                        _            )
;   (erbb2   _                       (KpbindF KpbindR KDepho2C)_                        _            )
;   (erbb3   _                        _                       (KpbindF KpbindR KDepho3C)_            )
;   (erbb4   _                        _                        _                       (KpbindF KpbindR KDepho4C)))
...
;   (.set-rate-function 'mass-action :fwd $K1f :rev $K1r)]
;  [{[[$R psite.p CPASE.1 **][$P CERB.1]] ->> [$R psite.u CPASE._ **] + [$P CERB._] @ :inner}
;   (.set-rate-function 'mass-action $K2f)])
;  [{[$R psite.p CSHC._ CGRB2._ CPI3K._ CPASE._ **] + [$P CERB._] @ :inner <<->> [[$R psite.p CSHC._ CGRB2._ CPI3K._ CPASE.1 **][$P CERB.1]]}
;   (.set-rate-function 'mass-action :fwd $K1f :rev $K1r)]
;  [{[[$R psite.p CSHC._ CGRB2._ CPI3K._ CPASE.1 **][$P CERB.1]] ->> [$R psite.u CSHC._ CGRB2._ CPI3K._ CPASE._ **] + [$P CERB._] @ :inner}
;   (.set-rate-function 'mass-action $K2f)])
...
;{[ADP].(in cell.cytoplasm).moles.t0 :=         0.0      } ; (#/pL)
...
(create-numerica-model "input/erb6_rcpt" :ode-comments nil :overwrite t :vars (query species.conc)
		       :sim-options '("CSVOUTPUT := TRUE" "DYNAMIC_REPORTING_INTERVAL := .1") :sim-steps 1800)
...
;(with-substitution-table 
;    ((  $R1      $R2    $Kf $Kr) 
;     (psite1.p psite2.u  KphoF KphoR)
;     (psite1.u psite2.p  KphoF KphoR)
;     (psite1.p psite2.p  KphoF KphoR))
...
;   ((psite1.p psite2.u)             _                     _            (KphoF KphoR))
;   ((psite1.u psite2.p)             _                     _            (KphoF KphoR))
;   ((psite1.p psite2.p)    (KphoF KphoR) (KphoF KphoR)          _          ))
...
;(with-data-table (:rows ($R1 $R2) :cols ($P1 $P2) :cells ($Kf $Kr) :ignore _)
;  ((                       (psite1.p psite2.u)    (psite1.u psite2.p)   (psite1.p psite2.p))
;   ((psite1.u psite2.u)    (KphoF KphoR)  (KphoF KphoR)          _        )
;   ((psite1.u psite2.p)             _                    _              (KphoF KphoR))
;   ((psite1.p psite2.u)             _                    _              (KphoF KphoR)))
...
;  ((                       (psite1.p psite2.u)     (psite1.u psite2.p)     (psite1.p psite2.p))
;   ((psite1.u psite2.u)    (1 1 1)                 (1 1 1)                          _         )
;   ((psite1.u psite2.p)               _                       _            (1 1 1)            )
;   ((psite1.p psite2.u)               _                       _            (1 1 1)            ))
...
;  (.set-rate-function 'mass-action :rev 1 :fwd 1)]
...
;(with-data-table (:rows ($RP1 $RP2) :cols ($PP1 $PP2) :cells ($Kf $Kr $Kc) :ignore _)
;  ((                     (psite1.u psite2.p)   (psite1.p psite2.u)  (psite1.u psite2.u))
;   ((psite1.p psite2.u)           _                     _           (KphoF KphoR KphoF))
;   ((psite1.u psite2.p)           _                     _           (KphoF KphoR KphoF))
;   ((psite1.p psite2.p)  (KphoF KphoR KphoF) (KphoF KphoR KphoF)         _          ))
